...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        best_word_count = 0
        best_shift = 0
        new_decrypted_word_list = []
        for s in range(26):
            message_text = self.get_message_text()
            message_text = self.apply_shift(s)
            decrypted_word_list = message_text.split(" ")
            valid_word_counter = 0 
            for word in decrypted_word_list:
                if is_word(word_list, word):
                    valid_word_counter += 1
            if best_word_count < valid_word_counter:
                best_word_count = valid_word_counter
                best_shift = s
                new_decrypted_word_list = self.apply_shift(s)
        return (best_shift, new_decrypted_word_list)
...
def decrypt_story():
    ...
    story_plain = story_cipher.decrypt_message()
    return story_plain
