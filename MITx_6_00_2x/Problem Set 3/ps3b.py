...
class Patient(object):
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try:
                virus_children.append(virus.reproduce(self.popDensity))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # for each trial in numTrials:
    patients_list = []
    for trial in range(numTrials):
        # instantiate numViruses Viruses with random maxBirthProb(up to 0.1) and random clearProb(up to 0.05)
        viruses_list = []
    ...
    for patient in patients_list:
        patient_virus_population = []
        for timestep in range(steps):
            patient_virus_population.append(patient.update())
        virus_population_per_patient_per_timestep.append(patient_virus_population)
    ...
    # create graph virus growth over timesteps
    pylab.figure()
    pylab.plot(averages_of_virus_population_per_timesteps, label='Virus Population')
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        resistant_to_all_activeDrugs = []
        for drug in activeDrugs:
            resistant_to_all_activeDrugs.append(self.isResistantTo(drug))
        if False in resistant_to_all_activeDrugs:
            raise NoChildException
        else:
            for drug in self.resistances:
                if random.random() <= self.mutProb:
                    self.resistances[drug] = False if self.resistances[drug] == True else True
            if random.random() <= (self.maxBirthProb * (1 - popDensity)):
...
class TreatedPatient(Patient):
        ...
        for virus in self.viruses:
            ...
            for drug in drugResist:
                if virus.isResistantTo(drug):
                    is_resistant.append(True)
                else:
                    is_resistant.append(False)
            if not False in is_resistant:
    ...
    def update(self):
        ...
        for virus in self.viruses:
            try:
                virus_children.append(virus.reproduce(self.popDensity, self.activeDrugs))
            except NoChildException:
                pass
        self.viruses += virus_children
        return self.getTotalPop()
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    patients_list = []
    for trial in range(numTrials):
        # instantiate numViruses ResistantVirus with maxBirthProb, clearProb, resistances and mutProb
        viruses_list = []
        for virus in range(numViruses):
            viruses_list.append(ResistantVirus(maxBirthProb, clearProb, resistances, mutProb))
        # instantiate 1 Patient per trial with numViruses Viruses and maxPop
        patients_list.append(TreatedPatient(viruses_list, maxPop))

    # run simulation 2 times: execute 150 timesteps each: call patient.update() 150 times each
    virus_population_per_patient_per_timestep = []
    resistant_virus_population_per_patient_per_timestep = []
    for patient in patients_list:
        patient_virus_population = []
        patient_resistant_virus_population = []
        for timestep in range(steps):
            patient_virus_population.append(patient.update())
            patient_resistant_virus_population.append(patient.getResistPop(['guttagonol']))
        else:
        ...
        for timestep in range(steps):
            ...
            virus_population_per_patient_per_timestep.append(patient_virus_population)
            resistant_virus_population_per_patient_per_timestep.append(patient_resistant_virus_population)
    ...
    pylab.plot(avg_rvpop_grader, label='Guttagonol Resistant')
